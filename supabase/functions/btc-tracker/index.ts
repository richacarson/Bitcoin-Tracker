...
async function fetchKrakenTransfers(creds: any, sinceSec = 0) {
  const transfers: any[] = [];
  let ofs = 0;
  for (;;) {
    const params: Record<string, string> = { asset: 'XBT', ofs: String(ofs) };
    if (sinceSec) params.start = String(sinceSec);
    const result = await krakenPrivate(creds, 'Ledgers', params);
    const entries = Object.entries(result.ledger || {});
    for (const [lid, l] of entries as [string, any][]) {
      if (l.type !== 'withdrawal' && l.type !== 'deposit') continue;
      transfers.push({
        id: `kraken:${lid}`, source: 'kraken',
        date: new Date(l.time * 1000).toISOString(),
        type: l.type === 'withdrawal' ? 'send' : 'receive',
        btc: Math.abs(parseFloat(l.amount)),
