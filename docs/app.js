...
function renderConnections() {
  ...
  fetch(API('api/settings'), { headers: authHeaders() }).then((r) => r.json()).then((s) => {
...
async function load() {
  ...
  try {
    ...
    if (res.status === 401) { localStorage.removeItem('btc_token'); location.href = 'login.html'; return; }
...
$('sync-btn').addEventListener('click', async () => {
  ...
  $('main').classList.add('refetching');
  try {
    await fetch(API('api/sync'), { method: 'POST', headers: authHeaders() });
    await load();
  } finally {
    btn.disabled = false;
