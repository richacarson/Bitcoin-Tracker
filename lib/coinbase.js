...
function buildJwt(creds, method, pathname) {
  const keyName = creds.keyName;
...
async function cbGet(creds, pathWithQuery) {
  const pathname = pathWithQuery.split('?')[0];
  const res = await fetch(`https://${HOST}${pathWithQuery}`, {
    headers: {
      Authorization: `Bearer ${buildJwt(creds, 'GET', pathname)}`,
...
async function cbGetAll(creds, firstPath) {
  const items = [];
  let next = firstPath;
  while (next) {
...
async function btcAccounts(creds) {
  const accounts = await cbGetAll(creds, '/v2/accounts?limit=100');
...
export async function fetchCoinbaseHistory(creds) {
  const trades = [];
  const transfers = [];
  let balance = 0;

  for (const account of await btcAccounts(creds)) {
    ...
    const txs = await cbGetAll(creds, `/v2/accounts/${account.id}/transactions?limit=100`);
    for (const t of txs) {
...
export const coinbaseConfigured = (creds) => Boolean(creds?.keyName && creds?.privateKey);
