...
// Kraken's private-endpoint rate limiter: starter-tier accounts decay just
// 0.33 points/sec and history endpoints cost 2 points per call, so anything
// faster than one call per ~6s eventually saturates the counter.
...
async function krakenPrivate(creds, endpoint, params = {}) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (errors.some((e) => e.includes('Rate limit'))) {
      await sleep(15000 * (attempt + 1)); // let the counter decay properly
      continue;
    }
    if (errors.length) throw new Error(`Kraken ${endpoint}: ${errors.join(', ')}`);
    return body.result;
  }
  const err = new Error(`Kraken ${endpoint}: rate limited after retries`);
